...
public class ResultRecord {
    ...
    public Match getMatch() {
        ...
        match.setSeasonId(getSeasonId());
        match.setLeagueType(LeagueType.valueOf(getLeagueType()));
        match.setRoundId(getRoundId());
        match.setDayId(getSeasonDayId());
        match.setDayDate(getSeasonDayDate());
        match.setLocalTeamId(getLocalTeamId());
        match.setLocalTeamName(getLocalTeamName());
        match.setLocalTeamScore(getLocalTeamScore());
        match.setLocalTeamPenaltyPoint(
            (getLocalTeamPenaltyPoint() == null?
                0 : getLocalTeamPenaltyPoint()));
        match.setVisitingTeamId(getVisitingTeamId());
        match.setVisitingTeamName(getVisitingTeamName());
        match.setVisitingTeamScore(getVisitingTeamScore());
        match.setVisitingTeamPenaltyPoint(
            (getVisitingTeamPenaltyPoint() == null?
                0: getVisitingTeamPenaltyPoint()));
        return match;
    }
    ...
    public MatchPK getMatchPK() {
        return new MatchPK(
            getSeasonId(),
            LeagueType.valueOf(getLeagueType()),
            getRoundId(),
            getSeasonDayId(),
            getSeasonDayDate(),
            getLocalTeamId(),
    ...
    public Table getTable() {
        ...
        if (StringUtil.isNullOrEmpty(getLocalPlayerNickOne())
            && StringUtil.isNullOrEmpty(getLocalPlayerNickTwo())) {
            table.setLocalWO(Table.ABSENT_WO); // no presentado
            table.setLocalPlayerNickOne(Player.ABSENT_PLAYER_ONE_NICK);
            table.setLocalPlayerNickTwo(Player.ABSENT_PLAYER_TWO_NICK);
        } else {
            table.setLocalWO(Table.PRESENT_WO);
            table.setLocalPlayerNickOne(getLocalPlayerNickOne());
            table.setLocalPlayerNickTwo(getLocalPlayerNickTwo());
        }
        table.setLocalPairScore(getLocalPairScore());
        
        if (StringUtil.isNullOrEmpty(getVisitingPlayerNickOne())
            && StringUtil.isNullOrEmpty(getVisitingPlayerNickTwo())) {
            table.setVisitingWO(Table.ABSENT_WO); // no presentado
            table.setVisitingPlayerNickOne(Player.ABSENT_PLAYER_ONE_NICK);
            table.setVisitingPlayerNickTwo(Player.ABSENT_PLAYER_TWO_NICK);
        } else {
            table.setVisitingWO(Table.PRESENT_WO);
            table.setVisitingPlayerNickOne(getVisitingPlayerNickOne());
            table.setVisitingPlayerNickTwo(getVisitingPlayerNickTwo());
        }
        table.setVisitingPairScore(getVisitingPairScore());
    ...
    public String toString() {
        ...
        sb.append((seasonDayDate == null?
            ...
            : StringUtil.enclose(
                CocamDatatypeConverter.printLocalDate(seasonDayDate))));
        sb.append(",matchHead");
    ...
    public static ResultRecord parse(String record,
        ...
        for (FieldDesc fieldDesc : sepColRecordDesc.getFieldDesc()) {
            ...
            Method method = null;
            try {
                method = ResultRecord.class.getMethod(methodName, String.class);
